...
function milspec(){
	...
	$count = count($arr['milspec']);
	for ($i=0; $i<$count; $i++) {
		include '/home/russianz/web/case.russianz.ru/public_html/config.php';
		$db = mysql_connect($config['db']['host'],$config['db']['user'],$config['db']['pass']);
		mysql_select_db($config['db']['base'],$db);
		$name_f = $arr['milspec'][$i][0].' | '.$arr['milspec'][$i][4];
		$sql = "SELECT COUNT(name) FROM `bot` WHERE `name` = \"{$name_f}\" OR `name` = \"StatTrak™ {$name_f}\""; mysql_query("SET NAMES 'utf8'"); mysql_query("SET CHARACTER SET 'utf8'");
			...
			$total = $row[0]; 
			if($total == 0){
				$first = $arr['milspec'][$i][0];
				if($arr['milspec'][$i][0] == 'PP-Bizon') $first = 'ПП-19 Бизон';
				check_price($first.' | '.$arr['milspec'][$i][1]);
				echo '+1';
				$c++;
			}
	}
	if($c>0){
		mysql_query("UPDATE `cases` SET `status`='0' WHERE `name` = 'milspec'");
	}
	return true;
}
