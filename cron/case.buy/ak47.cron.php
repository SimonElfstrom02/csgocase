...
function ak47(){
	...
	$count = count($arr['AK-47 Case']);
	for ($i=0; $i<$count; $i++) {
		include $_SERVER["DOCUMENT_ROOT"] . '/config.php';
		$db = mysql_connect($config['db']['host'],$config['db']['user'],$config['db']['pass']);
		mysql_select_db($config['db']['base'],$db);
		$name_f = $arr['AK-47 Case'][$i][0].' | '.$arr['AK-47 Case'][$i][4];
		$sql = "SELECT COUNT(name) FROM `bot` WHERE `name` = \"{$name_f}\" OR `name` = \"StatTrak™ {$name_f}\""; mysql_query("SET NAMES 'utf8'"); mysql_query("SET CHARACTER SET 'utf8'");
			...
			$total = $row[0];
			if($total == 0){
				check_price($arr['AK-47 Case'][$i][0].' | '.$arr['AK-47 Case'][$i][1]);
				$c++;
			}
	}
	if($c>0){
		mysql_query("UPDATE `cases` SET `status`='0' WHERE `name` = 'AK-47 Case'");
	}
	return true;
}
